...
class Gestures {
  async scrollToElement(element, maxScrolls, amount = 0) {
    // If the element is not displayed and we haven't scrolled the max amount of scrolls then scroll and execute the method again
    if (!(await element.isDisplayed()) && amount <= maxScrolls) {
      await this.swipeUp(0.85)
      await this.scrollToElement(element, maxScrolls, amount + 1)
  ...
  async swipeOnPercentage(from, to) {
    ...
    SCREEN_SIZE = SCREEN_SIZE || (await browser.getWindowRect())
